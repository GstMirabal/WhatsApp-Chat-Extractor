...
**Last Audit Commit SHA**: `4253873` (`ai-sprint/011` tip, per `docs/roadmaps/docs/extractor/002-delivery-program.md`; deployment pending — `main` is still at `136c1e1`, sealed `v0.9.0`)
...
**Released:** `v0.9.0` — Sprints 009+010 sealed (`CHANGELOG.md`): an append-only run journal with `--resume` and a browserless `recover` subcommand (`ADR-0006`), corpus contract v6 (`ADR-0007`), and `wa-extract consolidate` folding every per-chat export into one `data/corpus_<run_id>.ndjson`, verified live against 1018 real files. **Sprint 011 closed** on `ai-sprint/011` (Phase 7 `APPROVED`, Phase 8 closeout; deployment pending, not yet sealed as a version) — every item carried from Sprints 009-010: the harvest loop's wall-clock deadline (`--deadline-seconds`, `STOP_DEADLINE`, no longer blocks a fully unattended run), the journal header regression (`read_journal` now keeps the first `--resume` header, not the last), `__main__.py` split into CLI wiring plus a new `commands.py` (854→277 lines), and the title index rewritten append-only (`chat_index_<run_id>.ndjson`, replacing the batched writer). **In flight:** none — no sprint has started after 011; its own carried item (the `--deadline-seconds` end-to-end coverage gap) is unscheduled, not in progress.  
