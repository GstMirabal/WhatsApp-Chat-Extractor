...
| `KI-009-H` | **Found in production, not in review.** The harvest loop has no timeout of any kind: `collect_visible_rows`, `at_chat_start` and `panel_is_loading` are built on `query_selector`, `query_selector_all`, `inner_text` and `evaluate`, none of which accept a timeout, and no `set_default_timeout` is set on the context or page anywhere in `session.py`. When the Mac slept on 2026-09-03 and the Chromium renderer wedged, the next protocol call blocked forever: process alive, no log line, no journal record, for 5h 43m until the operator killed it. Run `20260902T214002Z` stopped at conversation 180 of 945 with **zero failures recorded**, because a hang is not a failure. **This defeats the resume work at its own premise**: the journal makes a *dead* run recoverable, and a *hung* run writes nothing at all — it is indistinguishable from a slow one except to a human watching. A per-conversation wall-clock deadline is needed, outside the page API, since `set_default_timeout` does not govern the `ElementHandle` calls this loop is built from | `host` | Sprint 010, blocking |
